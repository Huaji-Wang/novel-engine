...
悬念是让读者继续阅读的关键，但**不必每章结尾都吊 cliffhanger**。

## 节奏策略（Writer / 细纲须遵守）

1. **强钩子**：约每 **2–3 章** 设一次即可（危机、选择、情绪余波、关系变化、未完成目标等）。
2. **无强钩子的章节**：可以场景余韵、对话收束、小事件落定等方式结束；靠情节推进与人物变化维持可读性，不要硬凑「突然揭示」。
3. **弧末必钩**：当前章若是**本弧最后一章**，章末**必须**留强钩子，承接下一弧或卷（未闭合目标、关系转折、危机升级等）。
4. 十种钩子类型供轮换使用，避免连续多章同一模式（如章章「发现秘密」）。
...
3. **过度留白** - 留下太多未回答问题
   > 原则：每 **2–3 章** 宜推进或回应一条悬念线；不必每章都抛新钩子。弧末须集中留下强承接。
...
| 等级 | 类型 | 读者反应 | 适用位置 |
|-----|------|---------|---------|
| 1 | 好奇悬念 | "这很有趣" | 中间章节（非钩章节也可） |
| 2 | 关切悬念 | "接下来会发生什么" | 约每 2–3 章一次的强钩子 |
| 3 | 迫切悬念 | "他必须马上行动" | 弧内高潮 / 弧末强钩子 |
| 4 | 生存悬念 | "他会活下去吗" | 弧末 / 卷末 |
| 5 | 终极悬念 | "一切到底是什么意思" | 全书结尾 |

**递进建议：** 悬念强度总体可上升，但允许波动；**弧末**应达到本弧内较高强度。
